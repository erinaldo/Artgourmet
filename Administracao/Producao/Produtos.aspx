...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function pegaValoresLinha(linha) {
            var data = new Array();
        ...
        var FormularioCarregado;
        var StoreFornecedoresLoad = false;
        var StoreObservacoesLoad = false;
        var StoreListaAdicionaisLoad = false;
        var StoreInfoEstoqueLoad = false;
        var StorePlanosLoad = false;
        ...
        function filtraGridPrincipal() {
            ...
            if(GridPrincipal.getFilterPlugin().getFilter(campo) != undefined)
            {
                GridPrincipal.getFilterPlugin().getFilter(campo).setValue(filtro);
                GridPrincipal.getFilterPlugin().getFilter(campo).setActive(true);
            }
        }
        ...
        function FiltraUnidadeFichaTecnica(id, reseta) {

            var undControle = StorePrincipal.getById(id).data.undControle;
           
            StoreUndFT.filter('codUndBase', undControle,true,false);
            
            if (reseta) {
                alert('asdasd');
                ComboBoxIUnd.selectByIndex(0);
                ComboBoxIUnd.fireEvent('select');
            }
        }

        function CarregaItemFichaTecnica(rowIndex) {
            tipo_produto.setValue('1');

            FormPanelItemFicha.index = rowIndex;
            
            JanelaFichaTecnica.show();

            FormPanelItemFicha.getForm().loadRecord(StoreRecursos.getAt(rowIndex));

            var undControle = StorePrincipal.getById(ComboBoxProduto.getValue()).data.undControle;
            
            StoreUndFT.filter('codUndBase', undControle, true, false);
        }

        function AdicionarItemFichaTecnica() {
            var record = new StoreRecursos.recordType(); 

            var produto = ComboBoxProduto.getText();

            record.data.IdProduto = ComboBoxProduto.getValue();
            record.data.Codigo = produto.split('-')[0].trim();
            record.data.Nome = produto.split('-')[1].trim();
            record.data.idEmpresa = 0;
            record.data.Quantidade = txtI_Quantidade.getValue();
            record.data.Unidade = ComboBoxIUnd.getValue();
            record.data.ValorUnitario = parseFloat(txtIPrecoUnitario.getValue().replace('R$', '').trim().replace(',', '.'));
            record.data.ValorTotal = record.data.Quantidade * record.data.ValorUnitario;

            StoreRecursos.add(record);
            
            JanelaFichaTecnica.hide();
        }

        function AtualizarItemFichaTecnica()
        {
            var record = StoreRecursos.getAt(FormPanelItemFicha.index);
            var produto = ComboBoxProduto.getText();

            record.set('IdProduto',ComboBoxProduto.getValue());
            record.set('Codigo',produto.split(' - ')[0].trim());
            record.set('Nome',produto.split(' - ')[1].trim());
            record.set('Quantidade', txtI_Quantidade.getValue());
            record.set('Unidade', ComboBoxIUnd.getValue());
            record.set('ValorUnitario', parseFloat(txtIPrecoUnitario.getValue().replace('R$', '').trim().replace(',', '.')));
            record.set('ValorTotal', record.data.Quantidade * record.data.ValorUnitario);
            
            JanelaFichaTecnica.hide();
        }
        
        function CarregaItemPreco(rowIndex) {

            FormPanelPreco.index = rowIndex;

            JanelaPreco.show();

            FormPanelPreco.getForm().loadRecord(StorePrecos.getAt(rowIndex));

            tipo_preco.setValue('1');//EDIÇÃO
        }

        function AdicionarItemPreco() {
            var record = new StorePrecos.recordType();

            record.data.idEmpresa = StorePrincipal.getAt(0).data.idEmpresa;
            record.data.idFilial = ComboBoxFilialPreco.getValue();
            record.data.idTabPreco = -1;
            record.data.descricao = txtP_Descricao.getValue();
            record.data.preco1 = isNumber(txtP_Preco1.getValue()) ? txtP_Preco1.getValue() : '0';
            record.data.preco2 = isNumber(txtP_Preco2.getValue()) ? txtP_Preco2.getValue() : '0';
            record.data.preco3 = isNumber(txtP_Preco3.getValue()) ? txtP_Preco3.getValue() : '0';
            record.data.ativo = chkP_Ativo.checked;
            record.data.ativoDesc = record.data.ativo ? 'Ativo' : 'Inativo';
            
            StorePrecos.add(record);

            JanelaPreco.hide();
        }

        function AtualizarItemPreco() {
            var record = StorePrecos.getAt(FormPanelPreco.index);

            record.set('idEmpresa', StorePrincipal.getAt(0).data.idEmpresa);
            record.set('idFilial', ComboBoxFilialPreco.getValue());
            record.set('descricao', txtP_Descricao.getValue());
            record.set('preco1', isNumber(txtP_Preco1.getValue()) ? txtP_Preco1.getValue() : '0');
            record.set('preco2', isNumber(txtP_Preco2.getValue()) ? txtP_Preco2.getValue() : '0');
            record.set('preco3', isNumber(txtP_Preco3.getValue()) ? txtP_Preco3.getValue() : '0');
            record.set('ativo', chkP_Ativo.checked);
            record.set('ativoDesc',record.data.ativo ? 'Ativo' : 'Inativo');
...
<body onload="trocaTema()">
        ...
        <ext:Store ID="StorePrincipal" runat="server">
            <Reader>
                ...
                <DataChanged Handler=" if(JanelaPrincipal.isVisible())
                {
                                ...
                                if(FormularioCarregado != record.get('codigo'))
                                {
                                        FormularioCarregado = record.get('codigo');
                                        ...
                                        StorePrecos.reload();
                                        StoreFornecedoresLoad = false;
                                        StoreObservacoesLoad = false;
                                        StoreListaAdicionaisLoad = false;
                                        StoreInfoEstoqueLoad = false;
                                        StorePlanosLoad = false;
                                        ...
                                        TabPanelForm.activeTab.fireEvent('activate')
               ...
               <Add Handler="PagingToolbar1.changePage(StorePrincipal.getTotalCount());" />
            </Listeners>
        </ext:Store>
            ...
            <Reader>
                <ext:JsonReader>
                    <Fields>
                        ...
                        <ext:RecordField Name="Codigo" Type="String" />
                        <ext:RecordField Name="Nome" Type="String" />
                        <ext:RecordField Name="Quantidade" Type="Float" />
                        <ext:RecordField Name="Unidade" Type="String" />
                        <ext:RecordField Name="ValorUnitario" Type="Float"/>
        ...
        <ext:Store ID="StorePrecos" runat="server"  AutoLoad="false" OnRefreshData="StorePrecos_RefreshData">
            <Reader>
                <ext:JsonReader IDProperty="idTabPreco">
                    <Fields>
                        <ext:RecordField Name="idEmpresa" Type="Int" />
                        <ext:RecordField Name="idTabPreco" Type="Int" />
                        <ext:RecordField Name="idProduto" Type="Int" />
                        <ext:RecordField Name="idFilial" Type="Int" />
                        <ext:RecordField Name="descricao" Type="String" />
                        <ext:RecordField Name="preco1" Type="Float" />
                        <ext:RecordField Name="preco2" Type="Float" />
                        <ext:RecordField Name="preco3" Type="Float" />
                        <ext:RecordField Name="ativo" Type="Boolean" />
                        <ext:RecordField Name="ativoDesc" Type="String" />
                    </Fields>
                </ext:JsonReader>
        ...
        <ext:Store ID="StoreUndFT" runat="server" AutoLoad="True">
            <Reader>
                ...
                <ext:TabPanel EnableTabScroll="true" ID="TabPanelForm" runat="server" Height="470" DeferredRender="false"  >
                    <Items>
                                            ...
                                            <Items>
                                                <ext:Portlet ID="Portlet1" runat="server" Padding="5" Title="Dados" Icon="DateGo">
                                                    <Items>
                                                        ...
                                                        <ext:TextField ID="txtNome" LabelAlign="Top" runat="server" Height="40" Width="433" EmptyText="Digite o nome do produto" FieldLabel="Nome" AllowBlank="true" DataIndex="nome" />
                                                        
                                                         <ext:Container ID="CompositeField7" runat="server" Layout="ColumnLayout" Height="60">
                                                            <Items>
                                                                <ext:Panel ID="Panel4" runat="server" Width="277" Border="false" Height="43" Header="false" ColumnWidth=".65" Layout="Form" LabelAlign="Top">
                                                                    <Items>
                                                                        ...
                                                                        <Listeners>
                                                                            <TriggerClick Handler="this.clearValue(); StoreObservacoes.removeAll();" />
                                                                            <Change Handler="StoreObservacoes.reload(); StoreObservacoesLoad = true;" />
                                                                        </Listeners>
                                                                        </ext:ComboBox>
                                                                    </Items>
                                                                </ext:Panel>
                                                        ...
                                                        <ext:NumberField ID="txtValidade" runat="server" Width="150" FieldLabel="Validade(dias)" MaxLength="9" DataIndex="diasValidade" Margins="20px 0 0 0" Height="30"  />

                                                        
                                                        <ext:Container ID="CompositeField3" runat="server" Height="60">
                                                            <Items>
                                                                <ext:RadioGroup ID="rdTributacao" runat="server" AnchorHorizontal="100%" FieldLabel="Tributação" LabelAlign="Left" LabelPad="0" LabelWidth="65" DataIndex="tipoTributacao" LabelStyle="color:red;" ColumnsNumber="2">
                                                                    <Items>
                                                                         <ext:Radio ID="Radio2" Name="Tributacao" runat="server" BoxLabel="Tributado" InputValue="T" Width="40">
                                                                            <Listeners>
                                        ...
                                        <ext:PortalColumn ID="PortalColumn2" runat="server" ColumnWidth="0.4" DefaultAnchor="100%"
                                            ...
                                            <Items>
                                                <ext:Portlet ID="Portlet4" runat="server" Padding="5" Title="Miniatura" Icon="Image">
                                                    <Items>
                                                        <ext:Hidden ID="Hidden2" runat="server" DataIndex="idImagem">
                                                              <Listeners>
                                                                  <Change Handler="
                                                                          var _url = '../Sistema/CarregaImagem.aspx?id='+this.getValue();
                                                                          #{imgFotoProduto}.setImageUrl(_url);
                                                ...
                                                <ext:Portlet ID="Portlet3" runat="server" Padding="5" Width="100" Title="Informações" Icon="Information">
                                                    <Content>
                                                        <table cellspacing="0" align="center" class="tab" style="font-size: 15px; margin: 0 0 0 20px;">
                                                            <tr>
                                                                <td>Custo</td>
                                                                <td class="result">
                        ...
                        <ext:Panel ID="PanelFicha" HideMode="Offsets" runat="server" Title="Ficha Técnica" Cls="AddProduto" Height="510"
                            StyleSpec="padding:10px 10px 0 10px" >
                            <Items>
                            ...
                            <ext:GridPanel ID="GridFichaTecnica" Width="760" runat="server" Height="370" StoreID="StoreRecursos" AutoExpandColumn="Nome">
                                    <ColumnModel>
                                        <Columns>
                                            ...
                                            <ext:ImageCommandColumn Width="60px">
                                                <Commands>
                                                <%--Botões--%>
                                                    <ext:ImageCommand CommandName="editar" Icon="CommentEdit" Text="Editar">
                                                        <ToolTip Text="editar" />
                                                    </ext:ImageCommand>
                                                </Commands>
                                ...
                                <Listeners>
                                    <RowDblClick Handler="CarregaItemFichaTecnica(rowIndex);"></RowDblClick>
                                    <Command Handler="CarregaItemFichaTecnica(rowIndex);"></Command>
                                </Listeners>
                                </ext:GridPanel>        
                             </Items>
                        </ext:Panel>
                                ...
                                <ext:Portal ID="Portal3" runat="server" Layout="column" Height="380" Border="false">
                                    <Items>
                                                ...
                                                <ext:Portlet ID="Portlet6" Height="370" runat="server" Padding="5" Title="Receita" Icon="DateGo">
                                                    <Items>
                                                        <ext:NumberField ID="txtTempoPreparo" FieldLabel="Tempo de Finalização (mim)" 
                                                                         DataIndex="tempoPreparo" LabelWidth="170" 
                                                                         MaxLength="3" Width="220" runat="server"/>
                                        ...
                                        <ext:PortalColumn ID="PortalColumn7" runat="server" ColumnWidth="0.5" DefaultAnchor="100%"
                                            ...
                                            <Items>
                                                <ext:Portlet ID="Portlet9" runat="server" Padding="5" Title="Imagem" Icon="Image">
                                                    <Items>
                        ...
                        <%--Painel de Preços--%>
                        <ext:Panel ID="PanelPreco" HideMode="Offsets" runat="server" Title="Preços" Cls="AddProduto" StyleSpec="padding:10px 10px 0px 10px">
                            <Items>
                                <ext:GridPanel 
                                ID="GridPrecos" 
                                runat="server" 
                                ...
                                >
                                    <ColumnModel>
                                        <Columns>
                                            <ext:Column Header="Filial" DataIndex="idFilial" />
                                            ...
                                            <ext:Column Header="Preço 3" DataIndex="preco3">
                                             ...
                                             <ext:Column Header="Status" DataIndex="ativoDesc" />
                                            <ext:ImageCommandColumn Width="120">
                                                <Commands>
                                                    <ext:ImageCommand CommandName="preco" Icon="CommentEdit" Text="Editar">
                                                       <ToolTip Text="Preco" />
                                                    </ext:ImageCommand>
                                                </Commands>
                                            </ext:ImageCommandColumn>
                                    ...
                                    <Listeners>
                                        <RowDblClick Handler="CarregaItemPreco(rowIndex);"></RowDblClick>
                                        <Command Handler="CarregaItemPreco(rowIndex);"></Command>
                                    </Listeners>
                                </ext:GridPanel>
                            </Items>    
                        </ext:Panel>
                        ...
                        <ext:Panel ID="PanelFornecedores" HideMode="Offsets" runat="server" Title="Fornecedores" Cls="AddProduto" Layout="ColumnLayout" StyleSpec="padding:10px 10px 0px 10px">
                            <Items>
                            ...
                            <Listeners>
                                <Activate Handler="
                                        if(!StoreFornecedoresLoad)
                                        {
                                            PanelFornecedores.body.mask('Carregando...', 'x-mask-loading');
                                            StoreFornecedores.reload();
                                            StoreFornecedoresLoad = true;
                                        }">
                                </Activate>
                            </Listeners>
                        </ext:Panel>
                        ...
                        <ext:Panel ID="PanelObs" HideMode="Offsets" runat="server" Title="Observações" Cls="AddProduto" StyleSpec="padding:10px 10px 0px 10px">
                            <Items>
                            ...
                            <Listeners>
                                <Activate Handler="
                                        if(!StoreObservacoesLoad)
                                        {
                                            PanelObs.body.mask('Carregando...', 'x-mask-loading');
                                            StoreObservacoes.reload();
                                            StoreObservacoesLoad = true;
                                        }">
                                </Activate>
                            </Listeners>
                        </ext:Panel>
                        ...
                        <ext:Panel ID="PanelAdicionais" HideMode="Offsets" runat="server" Title="Adicionais" Cls="AddProduto" StyleSpec="padding:10px 10px 0px 10px">
                            <Items>
                            ...
                            <Listeners>
                                  <Activate Handler="
                                        if(!StoreListaAdicionaisLoad)
                                        {
                                            PanelAdicionais.body.mask('Carregando...', 'x-mask-loading');
                                            StoreListaAdicionais.reload();
                                            StoreListaAdicionaisLoad = true;  
                                        }">
                                </Activate>
                            </Listeners>
                        </ext:Panel>
                        ...
                        <ext:Panel ID="PanelMovimentacao" HideMode="Offsets" runat="server" Title="Movimentação do Produto" Cls="AddProduto" Hidden="true"
                            StyleSpec="padding:10px 10px 0px 10px">
                            <Items>
                             ...
                             <Listeners>
                                <Activate Handler="
                                        if(StoreInfoEstoqueLoad)
                                        {
                                            PanelMovimentacao.body.mask('Carregando...', 'x-mask-loading');
                                            StoreInfoEstoque.reload();
                                            StoreInfoEstoqueLoad = true;
                                        }">
                                </Activate>
                            </Listeners>
                        </ext:Panel>
                        ...
                        <ext:Panel ID="PanelFidelidade" HideMode="Offsets" runat="server" Title="Planos de Fidelidade" 
                           Cls="AddProduto" Hidden="true" StyleSpec="padding:10px 10px 0px 10px">
                            <Items>
                                <ext:GridPanel 
                            ...
                            <Listeners>
                                <Activate Handler="
                                        if(!StorePlanosLoad)
                                        {
                                            PanelFidelidade.body.mask('Carregando...', 'x-mask-loading');
                                            StorePlanos.reload();
                                            StorePlanosLoad = true;
                                        }">
                                </Activate>
                            </Listeners>
                        </ext:Panel>
            ...
            <TopBar>
                <ext:Toolbar ID="Toolbar6" runat="server" Flat="true">
                    <Items>
                        ...
                        <ext:PagingToolbar ID="PagingToolbar1" runat="server" PageSize="1" Flat="true" StoreID="StorePrincipal"
                            DisplayInfo="false">
                        </ext:PagingToolbar>
                    ...
                    <DirectEvents>
                        <Click OnEvent="btnOkFrm_Click">
                            ...
                            <ExtraParams>
                                ...
                                <ext:Parameter Name="FornecedoresLoad" Value="StoreFornecedoresLoad" Mode="Raw" />

                                <ext:Parameter Name="Adicionais" Value="Ext.encode(#{StoreListaAdicionais}.getRecordsValues())" Mode="Raw" />
                                <ext:Parameter Name="AdicionaisLoad" Value="StoreListaAdicionaisLoad" Mode="Raw" />

                                <ext:Parameter Name="Precos" Value="Ext.encode(pegaValoresLinha(#{StorePrecos}.getModifiedRecords()))" Mode="Raw" />

                                <ext:Parameter Name="Planos" Value="Ext.encode(#{GridPanelFidelidade}.getRowsValues())" Mode="Raw" />
                                <ext:Parameter Name="FidelidadeLoad" Value="StorePlanosLoad" Mode="Raw" />
                                
                                <ext:Parameter Name="ObservacoesLoad" Value="StoreObservacoesLoad" Mode="Raw" />
                            </ExtraParams>
                        </Click>
                    </DirectEvents>
                </ext:Button>
        ...
        Width="600">
            ...
            <Buttons>
             ...
             <Listeners>
                <Show Handler="SmProdutosAdicionais.clearSelections();" />
             </Listeners>
        </ext:Window>
        ...
        Width="400">
            <Items>
                <ext:FormPanel ID="FormPanelPreco" runat="server" Border="False" Width="370">
                    <Items>
                         <ext:ComboBox 
                    ...
                    <%--Campo Oculto--%>
                    <ext:TextField Hidden="true" ID="txtP_IdTabPreco" runat="server" DataIndex="idTabPreco" />
                   <ext:Hidden runat="server" ID="tipo_preco"></ext:Hidden>
                
                    <ext:TextField ID="txtP_Descricao" runat="server" Width="220" FieldLabel="Descrição" AllowBlank="true" DataIndex="descricao" />
                    <ext:NumberField ID="txtP_Preco1" runat="server" Width="180" FieldLabel="Preço 1" AllowBlank="true" MaxLength="9" DataIndex="preco1" />
                    <ext:NumberField ID="txtP_Preco2" runat="server" Width="180" FieldLabel="Preço 2" AllowBlank="true" MaxLength="9" DataIndex="preco2" />
                    <ext:NumberField ID="txtP_Preco3" runat="server" Width="180" FieldLabel="Preço 3" AllowBlank="true" MaxLength="9" DataIndex="preco3" />
                    <ext:Checkbox ID="chkP_Ativo" runat="server" FieldLabel="Ativo" DataIndex="ativo" />
        ...
        <ext:Window 
            ...
            Width="600">
            <Items>
                <ext:FormPanel ID="FormPanelItemFicha" runat="server" Width="600" Border="False">
                    <Items>
                    ...
                    <Listeners>
                          <AfterRender Handler="this.mun(this.el, 'keyup', this.onKeyUp, this);
                                                                          this.mon(this.el, 'keyup', onKeyUp, this);" />
                          <Select Handler="FiltraUnidadeFichaTecnica(this.getValue(),true);"></Select>
                    </Listeners>
                </ext:ComboBox>

                <ext:NumberField ID="txtI_Quantidade" runat="server" Width="180" FieldLabel="Quantidade" AllowBlank="true" IsRemoteValidation="true" MaxLength="9" AllowDecimals="True" DataIndex="Quantidade">
                ...
                <ext:ComboBox 
                    ...
                    <Listeners>
                        <Expand Handler="FiltraUnidadeFichaTecnica(ComboBoxProduto.getValue(),false)"></Expand>
                    </Listeners>
                </ext:ComboBox>

                <ext:DisplayField ID="txtIPrecoUnitario" runat="server" Width="350" FieldLabel="Custo Unitário" DataIndex="ValorUnitario" />

                <ext:DisplayField ID="txtITotal" runat="server" Width="350" FieldLabel="TOTAL" DataIndex="ValorTotal" />
                    </Items>
                </ext:FormPanel>
            ...
            <Buttons>
                <ext:Button ID="Button4" runat="server" Text="OK" Icon="Add" Width="80px">
                   <Listeners>
                       <Click Handler="
                           if(tipo_produto.getValue() == '1') {
                                AtualizarItemFichaTecnica();
                           }
                           else {
                                AdicionarItemFichaTecnica();
                           }
                           "></Click>
                   </Listeners>
                </ext:Button>
